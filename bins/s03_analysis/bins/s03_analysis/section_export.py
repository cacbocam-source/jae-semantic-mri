...
def infer_year_from_path(pdf_path: Path) -> int:
    ...
    This can be replaced later with explicit metadata from ledger/manifests.
    """
    name = pdf_path.name
    ...
    # fallback: try filename stem as int
    stem = pdf_path.stem
    try:
        return int(stem)
...
def build_section_export(pdf_path: str | Path) -> SectionExport:
    """
    Build a structured section export object for a PDF.
    """
...
def structured_output_path(pdf_path: str | Path) -> Path:
    """
    Map a raw PDF to its structured JSON output path.
    """
    path = Path(pdf_path).expanduser().resolve()
...
def write_section_export(pdf_path: str | Path) -> Path:
    """
    Build and write a structured section export JSON file.
    """
